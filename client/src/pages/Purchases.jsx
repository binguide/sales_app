...
import { Plus, Eye, Trash2, ArrowUpDown, ArrowRight, ClipboardList, Check, Undo2, Printer } from "lucide-react";
...
export default function Purchases() {
  ...
  const { sorted, toggle, getIndicator } = useSort(purchases, "id");
  ...
  function startNew() {
    setViewData(null);
    setSupplierId(""); setWarehouseId(""); setPaymentMethodId(""); setCurrencyId("");
  ...
  function onProductSelect(id) {
    ...
    if (p) {
      ...
      if (base) { setSelUnitId(String(base.unit_id)); setSelUnitFactor(base.conversion_factor); setSelPrice(String(base.price)); }
      else { setSelPrice(String(p.price)); }
      setSelTax(String(p.tax_rate || ""));
    }
  }

  function addItem() {
    if (!selProduct) return toast(t("purchases.noProduct"), { type: "error" });
    const p = products.find((x) => x.id === parseInt(selProduct));
    if (!p) return;
    const qty = parseInt(selQty) || 1;
    const price = parseFloat(selPrice) || 0;
    if (qty <= 0 || price <= 0) return toast(t("purchases.invalidQtyPrice"), { type: "error" });
  ...
  function savePurchase() {
    if (selectedItems.length === 0) return toast(t("purchases.emptyInvoice"), { type: "error" });
    api.addPurchase({
      ...
      toast(t("purchases.saved"), { type: "success" });
      setMode("list");
      load();
    });
  }
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-3xl font-bold tracking-tight">{t("purchases.newInvoice")}</h2>
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={savePurchase} disabled={selectedItems.length === 0}><Check className="h-4 w-4 ml-1" />{t("common.save")}</Button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <Combobox value={warehouseId} onChange={(e) => setWarehouseId(e.target.value)} placeholder="اختر...">
        ...
        <Card>
          <CardHeader className="py-3 flex flex-row items-center justify-between">
            <CardTitle className="text-base flex items-center gap-2"><ClipboardList className="h-4 w-4" />{t("sales.addItem")}</CardTitle>
          </CardHeader>
          <CardContent>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold tracking-tight">{t("purchases.title")}</h2>
          <p className="text-muted-foreground">{t("purchases.subtitle")}</p>
        </div>
        <div className="flex gap-2">
          ...
          ]} filename="purchases" title={t("purchases.title")} />
          <Button onClick={startNew}><Plus className="h-4 w-4 ml-2" />{t("purchases.newInvoice")}</Button>
        </div>
      </div>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <Th sortKey="supplier_name" toggle={toggle} indicator={getIndicator}>{t("purchases.supplier")}</Th>
              <TableHead>{t("common.total")}</TableHead>
          ...
          <TableBody>
            {sorted.length === 0 ? (
              ...
              return (
              <TableRow key={p.id}>
                <TableCell className="font-medium">#{fn0(p.id)}</TableCell>
                <TableCell>{p.supplier_name}</TableCell>
                <TableCell>{fn(p.total)} {p.currency_symbol}</TableCell>
                <TableCell className={paid >= p.total ? "text-green-600 font-medium" : paid > 0 ? "text-amber-600" : "text-muted-foreground"}>{fn(paid)}</TableCell>
                <TableCell className={remaining <= 0 ? "text-green-600" : "text-red-600 font-medium"}>{remaining <= 0 ? "—" : fn(remaining)}</TableCell>
                <TableCell>{new Date(p.date).toLocaleDateString(locale === "ar" ? "ar-SA" : "en-US")}</TableCell>
                <TableCell>
      ...
      <Dialog open={!!viewData} onOpenChange={(open) => !open && setViewData(null)} contentClassName="max-w-4xl max-h-[90vh] overflow-y-auto">
        {viewData && (
          <>
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2"><ClipboardList className="h-4 w-4" />{t("purchases.invoice")} #{viewData.id}</DialogTitle>
            </DialogHeader>
            <div className="flex gap-2 mb-4">
              <Button variant="outline" size="sm" onClick={() => printInvoice(viewData, "purchase", locale)}><Printer className="h-3.5 w-3.5 ml-1" />{t("common.print") || "طباعة"}</Button>
              <Button variant="outline" size="sm" onClick={() => navigate(`/purchase-returns?fromPurchase=${viewData.id}`)}><Undo2 className="h-3.5 w-3.5 ml-1" />مرتجع</Button>
            </div>
            <div className="grid grid-cols-2 gap-2 text-sm mb-4">
              <div><span className="text-muted-foreground">{t("purchases.supplier")}:</span> {viewData.supplier_name}</div>
            ...
            <Table>
              <TableHeader>
                <TableRow><TableHead>{t("common.name")}</TableHead><TableHead>{t("common.quantity")}</TableHead><TableHead>{t("products.unit")}</TableHead><TableHead>{t("common.price")}</TableHead>{viewData.items && viewData.items.some((i) => i.tax_rate > 0) && <TableHead>الضريبة</TableHead>}<TableHead>{t("common.total")}</TableHead></TableRow>
              </TableHeader>
              <TableBody>
            ...
            <Card className="mt-4">
              ...
              <CardContent>
                <div className="flex gap-4 mb-3 text-sm">
                  <span className="text-muted-foreground">{t("payments.totalPaid")}: <strong className={(viewData.total - (viewData.payments?.reduce((s, p) => s + p.amount, 0) || 0)) <= 0 ? "text-green-600" : "text-amber-600"}>{fn(viewData.payments?.reduce((s, p) => s + p.amount, 0) || 0)}</strong></span>
